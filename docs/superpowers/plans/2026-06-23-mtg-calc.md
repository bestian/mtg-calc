...
Note: castability first applies the turn-drop gate (`maxMana = playFirst ? roundIdx + 1 : roundIdx`). If `cmc > maxMana`, that round's castability is exactly 0. Otherwise it uses `landMatrix` for P(total lands ≥ CMC), and `colorMatrix` for P(color_c ≥ pip_c). This correctly uses actual land count, not the sum of specified colour counts.
