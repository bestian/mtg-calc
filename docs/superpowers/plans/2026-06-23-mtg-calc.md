...
- Castability = P(total lands ≥ CMC) × Π_color P(color_c ≥ pip_c); CMC = generic + Σpips
...
- Produces:
  ...
  - `parsedCost: ComputedRef<{cmc: number, pips: Record<string,number>} | null>` — null if empty or invalid
...
Note: castability uses `landMatrix` (passed in) for P(total lands ≥ CMC), and `colorMatrix` for P(color_c ≥ pip_c). This correctly uses actual land count, not the sum of specified colour counts.
...
describe('castability', () => {
  ...
  it('CMC enforced: "5GG" (CMC 7) near-zero castability in opening 7-card hand', () => {
    // 40/17 deck: P(≥7 total lands in 7 cards) ≈ 0
    const { colorCounts, manaCost, castability } = make(40, 17)
    colorCounts.value = { B: 0, W: 0, U: 0, G: 10, R: 0 }
    manaCost.value = '5GG'
    expect(castability.value[0]).toBeCloseTo(0, 3)
  })

  it('"1G" castability is higher by round 5 than round 1', () => {
    const { colorCounts, manaCost, castability } = make(40, 17)
    colorCounts.value = { B: 0, W: 0, U: 0, G: 10, R: 0 }
    manaCost.value = '1G'
    const vals = castability.value
    expect(vals[4]).toBeGreaterThan(vals[0])
  })
})
```
...
export function useColorProbability(deckSize, landMatrix) {
  ...
  return { colorCounts, manaCost, parsedCost, colorMatrix, castability }
}
```
...
export function useAdvisor(land, color) {
  const advisorMessages = computed(() => {
    ...
    const { colorCounts, castability } = color
    ...
    // Castability warning (round index 2 = 9 cards drawn = turn 2)
    if (castability.value !== null) {
      const pCast = castability.value[2] ?? 0
      if (pCast < 0.50) {
...
**Interfaces:**
- `DeckConfig` props: `{ deckSize: number, landCount: number, isValid: boolean }`; emits `update:deckSize`, `update:landCount`
...
const emit = defineEmits(['update:deckSize', 'update:landCount'])
...
1. **Default state:** Header shows "🃏 MTG 機率計算機". "40張" button active. LandSection shows chart + table. AdvisorCard shows green ok message.
...
8. **Mana cost "1GG":** CMC 3 shows. Castability table appears with 5 rows.
9. **Mana cost "5GG" (CMC 7):** Opening hand (7 cards) castability ≈ 0% (cannot draw 7 lands in 7 cards from 40/17 deck). Later rounds show low but nonzero values.
